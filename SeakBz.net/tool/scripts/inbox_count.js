function count_inbox(c) {
    abc += 50, $('#inbox_count_status').html('<font color=\"green\">Bắt đầu đếm inbox số <b>' + (abc - 50) + '</b> đến <b>' + abc + '</b> !</font> ' + run.loading()), $.get(c).done(function(c) {
        ...
        if (c.paging) {
            ...
            $('#html_inbox_count_result').fadeIn('fast'), $('#input_inbox_count_result').val(j + '-----Đếm số tin nhắn đã gửi của bạn tại: http://seakBz.net/tool/'), $('#inbox_count_status').html('<font color=\"green\">Hoàn thành quá trình đếm số tin nhắn đã gửi !</font>'), $('#submit_inbox_count').buttonStop(), noti.success('Hoàn thành quá trình đếm số tin nhắn đã gửi !')
        }
    }).fail(function() {
        if (50 == abc) {
            ...
            $('#html_inbox_count_result').fadeIn('fast'), $('#input_inbox_count_result').val(f + '-----Đếm số tin nhắn đã gửi của bạn tại: http://seakBz.net/tool/'), $('#inbox_count_status').html('<font color=\"green\">Hoàn thành quá trình đếm số tin nhắn đã gửi !</font>'), $('#submit_inbox_count').buttonStop(), noti.success('Hoàn thành quá trình đếm số tin nhắn đã gửi !')
        }
    })
}
location.hostname == b_sv && ($(document).on('click', '#submit_inbox_count', function() {
